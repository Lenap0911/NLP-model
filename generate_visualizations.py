...
def write_interpretations(df, topics_df):
    ...
    lines = [
        '# Flood NLP Pipeline — Results & Interpretations',
        '',
        '**Pipeline run:** enriched.csv',
        '',
        '---',
        '',
        '## Step 1 — Preprocessing',
        '',
        '- Language codes mapped from ISO 639-2 to ISO 639-1 via LANGUAGE_CODE_MAP in config.',
        '- `flood_term_hits` (pre-computed in CSV) used directly — no recomputation.',
        '- `is_content_duplicate` flag used for deduplication.',
        '',
        ...
        '',
        '- Cross-lingual pairs identified using CSLS scoring with percentile threshold.',
        '- The EN↔ES/PT comparison is the key analytical step for the Americas dataset.',
        '',
